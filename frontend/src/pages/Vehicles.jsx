...
export default function Vehicles() {
  ...
  const startEdit = (v) => {
    setEditingId(v._id);
    setForm({ vehicleId: v.vehicleId, plateNumber: v.plateNumber, model: v.model, vehicleType: v.vehicleType, seatingCapacity: v.seatingCapacity ?? 4, currentMileage: v.currentMileage, status: v.status });
    setShowForm(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      const payload = { ...form, seatingCapacity: Number(form.seatingCapacity), currentMileage: Number(form.currentMileage) };
      if (editingId) {
